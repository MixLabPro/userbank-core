...
@mcp.tool()
def manage_persona(action: str, name: str = None, gender: str = None, personality: str = None, 
                  avatar_url: str = None, bio: str = None, privacy_level: str = None) -> Dict[str, Any]:
    """用户画像管理工具。支持获取和保存操作。
    
    参数说明：
    - action: 操作类型，'get'(获取) 或 'save'(保存)
    
    获取操作 (action='get') 使用参数：
    - 无需其他参数，直接获取当前用户画像信息
    
    保存操作 (action='save') 使用参数：
    - name, gender, personality, avatar_url, bio, privacy_level: 用户画像字段
    - 只需提供需要修改的字段，系统中只有一个用户画像，ID固定为1
    """
    try:
        if action == "get":
...
- **内容相关性**: 结合用户画像信息，使AI的回答和建议更贴近用户需求。"""
            ...
            return {
                ...
                "operation": "error",
                "timestamp": datetime.now().isoformat(),
                "error": f"无效的操作类型: {action}，支持的操作: 'get', 'save'"
            }
...
def manage_viewpoints(action: str, id: int = None, content: str = None, source_people: str = None,
    ...
    保存操作 (action='save') 使用参数：
    - id: 记录ID，None表示创建新记录，有值表示更新现有记录
    - content, source_people 等: 观点数据字段
    """
    try:
        if action == "query":
            filter_conditions = {}
...
def manage_insights(action: str, id: int = None, content: str = None, source_people: str = None,
    ...
    保存操作 (action='save') 使用参数：
    - id: 记录ID，None表示创建新记录，有值表示更新现有记录
    - content, source_people 等: 洞察数据字段
    """
    try:
        if action == "query":
            filter_conditions = {}
...
def manage_goals(action: str, id: int = None, content: str = None, type: str = None, 
    ...
    保存操作 (action='save') 使用参数：
    - id: 记录ID，None表示创建新记录，有值表示更新现有记录
    - content, type, deadline 等: 目标数据字段
    """
    try:
        if action == "query":
            filter_conditions = {}
...
# ============ VII. Methodology Tools (合并后的单个工具) ============
...
def manage_methodologies(action: str, id: int = None, content: str = None, type: str = None,
    ...
    保存操作 (action='save') 使用参数：
    - id: 记录ID，None表示创建新记录，有值表示更新现有记录
    - content, type, effectiveness 等: 方法论数据字段
    """
    try:
        if action == "query":
            filter_conditions = {}
...
def manage_focuses(action: str, id: int = None, content: str = None, priority: int = None, 
    ...
    保存操作 (action='save') 使用参数：
    - id: 记录ID，None表示创建新记录，有值表示更新现有记录
    - content, priority, status 等: 关注点数据字段
    """
    try:
        if action == "query":
            filter_conditions = {}
...
def manage_predictions(action: str, id: int = None, content: str = None, timeframe: str = None, 
    ...
    保存操作 (action='save') 使用参数：
    - id: 记录ID，None表示创建新记录，有值表示更新现有记录
    - content, timeframe, basis 等: 预测数据字段
    """
    try:
        if action == "query":
            filter_conditions = {}
